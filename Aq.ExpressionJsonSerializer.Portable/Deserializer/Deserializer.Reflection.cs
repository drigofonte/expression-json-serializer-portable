...
namespace Aq.ExpressionJsonSerializer
{
    partial class Deserializer
    {
        ...
        private Type Type(JToken token)
        {
            ...
            if (!types.TryGetValue(typeName, out type)) {
                ...
                if (type == null) {
                    ...
                    {
                        var assembly = this._assemblies.SingleOrDefault(a => a.FullName.Equals(assemblyName) || a.FullName.Split(',')[0].Equals(assemblyName.Split(',')[0]));
                        if (assembly != null)
                            type = assembly.GetType(typeName);
                    }
                }
        ...
        private MethodInfo Method(JToken token)
        {
            ...
            var method = methods.First(m => m.Name == name && Regex.Replace(m.ToString(), @"\s+", "") == signature);
